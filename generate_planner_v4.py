"""
Pregnancy Planner — v4
======================
v4 design revision (per Annie, 29 May 2026) — review notes on v3:

  * Removed the horizontal icon nav strip from every page (side tabs stay).
  * Refreshed palette + typography toward the reference inspiration:
    elegant letter-spaced serif display titles, muted sage, warm clay,
    terracotta, plus navy + rose accents; a delicate single-stem botanical
    sprig replaces every "weird graphic" (cover, trimester / section /
    journal / notes / closing title pages).
  * Weekly pages: page title is now "Weekly Planner"; the left "Daily
    Planner" header is gone and the seven days stretch the full page
    height; section titles are larger and sit closer to their block; the
    bump box is renamed "Progress Photo" and is now portrait; the to-do
    column now carries Actions (checkboxes) + Research + Consider pulled
    from the Excel weekly-tasks list.
  * Bullet points aligned to their text throughout (about, preambles).
  * Appointment pages: "Questions to ask" is now a bulleted section.
  * Scan pages: scan photo box is now square.
  * Hospital bag + baby essentials spaced out to use the full page.
  * Signs of labour: checkboxes replaced with bullets.
  * Removed the two "questions" checklist pages (midwife / scan).
  * Birth plan: title reads "Vaginal or C-Section"; Environment is now a
    free-write section; "ice chips" removed; free lines after every
    section; C-section page spaced out.

----------------------------------------------------------------------
Prior v3 restructure (per Annie, 27 May 2026):
...
Output: pregnancy_planner_2026_v4.pdf
"""
...
# Muted, reference-inspired palette — sage / clay / terracotta trimester
# theming, warmed with navy + rose accents and soft cream.
SAGE         = HexColor("#8fa388")   # T1 — muted sage-grey green
CLAY         = HexColor("#c4956a")   # T2
TERRACOTTA   = HexColor("#b56e57")   # T3
OAT          = HexColor("#8c7a66")   # neutral sections
SAGE_DEEP    = HexColor("#6c8169")
CLAY_DEEP    = HexColor("#9e7449")
TERRA_DEEP   = HexColor("#925240")
OAT_DEEP     = HexColor("#6d5f4f")

# Reference accents
NAVY         = HexColor("#3f4a5a")   # deep slate — footers / display accents
NAVY_DEEP    = HexColor("#333c49")
ROSE         = HexColor("#a86b78")   # dusty rose — secondary accent
ROSE_DEEP    = HexColor("#8d5562")
BLUSH        = HexColor("#ead7d6")

CREAM        = HexColor("#faf6ef")
TEXT_DARK    = HexColor("#43403c")
TEXT_SEC     = HexColor("#9c9486")
...
TRIM_WHITE   = HexColor("#e7e7e3")
PHOTO_BG     = HexColor("#f7f2e8")
PHOTO_BORDER = HexColor("#d4c8b3")

# Display serif used for big titles (reference-style letter-spaced caps)
SERIF        = "Times-Roman"
...
BODY_START       = BAND_H + 18   # body starts just under the header band
...
def draw_block_title(c, label, top_y, margin_x=None, content_w=None,
                     accent=OAT, size=11, gap_after=15):
    """Larger, accent-coloured section title that sits close to its block
    (used on the weekly pages). A short accent rule sits under the word."""
    if margin_x is None: margin_x = MARGIN_X
    if content_w is None: content_w = CONTENT_W

    c.setFont("Helvetica-Bold", size)
    c.setFillColor(accent)
    c.drawString(margin_x, rl(top_y + size), label.upper())

    label_w = c.stringWidth(label.upper(), "Helvetica-Bold", size)
    c.setStrokeColor(accent)
    c.setLineWidth(1.0)
    c.line(margin_x, rl(top_y + size + 4), margin_x + min(label_w, content_w),
           rl(top_y + size + 4))
    return top_y + size + gap_after


def draw_bullet(c, text, top_y, margin_x, content_w, color=CLAY,
                size=10, leading=15, radius=2.2):
    """Bullet row with the dot vertically aligned to the first text line."""
    # First-line baseline is at rl(top_y); centre the dot on the line's
    # visual middle (~0.32 * font size above the baseline).
    c.setFillColor(color)
    c.circle(margin_x + radius, rl(top_y) + size * 0.32, radius, fill=1, stroke=0)
    indent = radius * 2 + 9
    return draw_wrapped(c, text, margin_x + indent, top_y, content_w - indent,
                        font="Helvetica", size=size, leading=leading,
                        color=TEXT_DARK)


def draw_tracked(c, text, x, baseline_y, font, size, tracking,
                 color=white, centre_x=None):
    """Draw text with manual letter-spacing (reportlab's canvas has no
    setCharSpace). If centre_x is given, the tracked string is centred
    on that x; otherwise it starts at x."""
    c.setFont(font, size)
    c.setFillColor(color)
    total = (sum(c.stringWidth(ch, font, size) for ch in text)
             + tracking * max(len(text) - 1, 0))
    cur = (centre_x - total / 2) if centre_x is not None else x
    for ch in text:
        c.drawString(cur, baseline_y, ch)
        cur += c.stringWidth(ch, font, size) + tracking


def draw_display_title(c, text, cx, baseline_y, size=34, color=white,
                       tracking=None, font=SERIF):
    """Centred, letter-spaced serif display title (reference style)."""
    if tracking is None:
        tracking = size * 0.14
    draw_tracked(c, text, 0, baseline_y, font, size, tracking,
                 color=color, centre_x=cx)
...
# DECORATIVE GRAPHICS
# ─────────────────────────────────────────────────────────────────────
def _leaf(c, bx, by, tx, ty, bulge):
    """Draw a single almond leaf outline from base (bx,by) to tip (tx,ty)."""
    dx, dy = tx - bx, ty - by
    L = (dx * dx + dy * dy) ** 0.5 or 1.0
    # unit perpendicular
    px, py = -dy / L, dx / L
    b1x, b1y = bx + dx * 0.25 + px * bulge, by + dy * 0.25 + py * bulge
    m1x, m1y = bx + dx * 0.55 + px * bulge, by + dy * 0.55 + py * bulge
    m2x, m2y = bx + dx * 0.55 - px * bulge, by + dy * 0.55 - py * bulge
    b2x, b2y = bx + dx * 0.25 - px * bulge, by + dy * 0.25 - py * bulge
    p = c.beginPath()
    p.moveTo(bx, by)
    p.curveTo(b1x, b1y, m1x, m1y, tx, ty)
    p.curveTo(m2x, m2y, b2x, b2y, bx, by)
    c.drawPath(p, fill=0, stroke=1)


# 5 unit directions for a small bloom
_PETALS5 = [(0.0, 1.0), (0.951, 0.309), (0.588, -0.809),
            (-0.588, -0.809), (-0.951, 0.309)]


def draw_sprig(c, cx, cy, color=white, scale=1.0, height=150):
    """A single delicate stem with leaf pairs and a small bloom on top.

    This is the one decorative motif used across all title / cover pages
    (replaces the old leaf / circle / arc / column graphics)."""
    s = scale
    c.setStrokeColor(color)
    c.setFillColor(color)
    c.setLineWidth(1.0)

    bottom = cy - height / 2 * s
    top    = cy + height / 2 * s

    # Gently curved main stem
    p = c.beginPath()
    p.moveTo(cx, bottom)
    p.curveTo(cx - 5 * s, cy - height * 0.15 * s,
              cx + 5 * s, cy + height * 0.15 * s, cx, top - 8 * s)
    c.drawPath(p, fill=0, stroke=1)

    # Leaf pairs angled gently upward along the stem
    for frac, ln in [(0.16, 30), (0.36, 36), (0.56, 32), (0.74, 24)]:
        ly = bottom + (top - bottom) * frac
        _leaf(c, cx, ly, cx - ln * s, ly + 14 * s, 5.5 * s)
        _leaf(c, cx, ly, cx + ln * s, ly + 14 * s, 5.5 * s)

    # Small bloom at the top
    bx, by = cx, top - 4 * s
    for ux, uy in _PETALS5:
        _leaf(c, bx, by, bx + ux * 13 * s, by + uy * 13 * s, 3.2 * s)
    c.circle(bx, by, 2.6 * s, fill=1, stroke=0)


# Back-compat aliases — every old motif now renders the single sprig.
def draw_botanical_motif(c, cx, cy, color=white, scale=1.0):
    draw_sprig(c, cx, cy, color=color, scale=scale)
...
def draw_cover_motif(c, cx, cy, color=white):
    draw_sprig(c, cx, cy, color=color, scale=1.1, height=150)
...
def draw_cover(c):
    ...
    page_cx = (PAGE_W - TAB_W) / 2   # centre of the usable (non-tab) area

    # Single delicate sprig sitting above the title
    draw_sprig(c, page_cx, PAGE_H / 2 + 150, color=white, scale=1.15,
               height=150)

    title_y_rl = PAGE_H / 2 - 6

    # Thin rule above title
    c.setStrokeColor(white)
    c.setLineWidth(0.5)
    c.line(page_cx - 150, title_y_rl + 52, page_cx + 150, title_y_rl + 52)

    # Letter-spaced serif title, on two lines
    draw_display_title(c, "THE PREGNANCY", page_cx, title_y_rl + 18,
                       size=30, color=white, font=SERIF)
    draw_display_title(c, "PLANNER", page_cx, title_y_rl - 20,
                       size=30, color=white, font=SERIF)

    # Subtitle
    c.setFillColor(TRIM_WHITE)
    c.setFont("Helvetica-Oblique", 12)
    c.drawCentredString(page_cx, title_y_rl - 50,
                        "A place to plan and prepare, week by week.")

    # Lower rule
    c.setStrokeColor(white)
    c.setLineWidth(0.5)
    c.line(page_cx - 150, title_y_rl - 68, page_cx + 150, title_y_rl - 68)

    # Navy footer bar (reference-style), spanning the usable width
    bar_h = 30
    c.setFillColor(NAVY)
    c.rect(0, 0, PAGE_W - TAB_W, bar_h, fill=1, stroke=0)
    draw_tracked(c, "WEEKS 4 – 40   ·   UK NHS PATHWAY", 0, bar_h / 2 - 3,
                 "Helvetica", 8, 2, color=white, centre_x=page_cx)
...
def draw_about_page(c):
    ...
    for bullet in bullets:
        cur_y = draw_bullet(c, bullet, cur_y, mx, cw, color=CLAY,
    ...
    for bullet in nav_bullets:
        cur_y = draw_bullet(c, bullet, cur_y, mx, cw, color=SAGE,
                            size=10.5, leading=16)
        cur_y += 11
...
def draw_trimester_title_page(c, trimester_name, week_range, color, motif,
    ...
    page_cx = (PAGE_W - TAB_W) / 2

    # Single delicate sprig above the title
    draw_sprig(c, page_cx, PAGE_H / 2 + 120, color=white, scale=1.15,
               height=140)

    # Big letter-spaced serif trimester name
    draw_display_title(c, trimester_name.upper(), page_cx, PAGE_H / 2 - 26,
                       size=34, color=white, font=SERIF)

    # Decorative line below
    c.setStrokeColor(white)
    c.setLineWidth(0.5)
    c.line(page_cx - 130, PAGE_H / 2 - 52, page_cx + 130, PAGE_H / 2 - 52)

    # Smaller week range subtitle
    draw_tracked(c, week_range.upper(), 0, PAGE_H / 2 - 76,
                 "Helvetica", 12, 1.5, color=TRIM_WHITE, centre_x=page_cx)
...
def draw_trimester_preamble(c, trimester_name, week_range, color,
    ...
    for i, pri in enumerate(priorities, 1):
        # Numbered circle — centred on the first text line
        num_cy = rl(cur_y) + 3.0
        c.setFillColor(color)
        c.circle(mx + 6, num_cy, 6.5, fill=1, stroke=0)
        c.setFillColor(white)
        c.setFont("Helvetica-Bold", 7.5)
        c.drawCentredString(mx + 6, num_cy - 2.6, str(i))
        c.setFillColor(TEXT_DARK)
        cur_y = draw_wrapped(c, pri, mx + 20, cur_y, cw - 20,
    ...
    for habit in habits:
        cur_y = draw_bullet(c, habit, cur_y, mx, cw, color=color,
                            size=10, leading=15)
        cur_y += 6
    cur_y += 6
...
def draw_weekly_page(c, week_num, data):
    color, _ = get_trimester_color(week_num)
    if week_num <= 12:   active_idx = 1
    elif week_num <= 26: active_idx = 2
    else:                active_idx = 3

    page_frame(c, label="Weekly Planner",
               sublabel=f"Week {week_num} · {get_trimester(week_num)}",
               active_idx=active_idx, band_color=color)

    # Column layout — narrow daily-planner column on the left
    left_col_w  = int(CONTENT_W * 0.35)
    col_gap     = 18
    ...
    body_top  = BODY_START
    bottom_ty = PAGE_H - BOTTOM_MARGIN

    # Faint vertical divider between the two columns
    div_x = lx + left_col_w + col_gap / 2
    c.setStrokeColor(LINE_MED)
    c.setLineWidth(0.5)
    c.line(div_x, rl(body_top), div_x, rl(bottom_ty))

    # ── LEFT COLUMN: the seven days, stretched full page height ─────
    days_labels = ["MONDAY", "TUESDAY", "WEDNESDAY", "THURSDAY",
                   "FRIDAY", "SATURDAY", "SUNDAY"]
    day_h = (bottom_ty - body_top) / 7
    ly = body_top
    for day_label in days_labels:
        c.setFont("Helvetica-Bold", 9)
        c.setFillColor(color)
        c.drawString(lx, rl(ly + 10), day_label)
        label_w = c.stringWidth(day_label, "Helvetica-Bold", 9)
        c.setStrokeColor(LINE_MED)
        c.setLineWidth(0.5)
        c.line(lx + label_w + 6, rl(ly + 6), lx + left_col_w, rl(ly + 6))

        line_spacing = 13
        n_lines = max(2, int((day_h - 18) / line_spacing))
        c.setStrokeColor(LINE_LIGHT)
        c.setLineWidth(0.4)
        for i in range(n_lines):
            yy = rl(ly + 16 + line_spacing * i)
            c.line(lx, yy, lx + left_col_w, yy)
        ly += day_h

    # ── RIGHT COLUMN ───────────────────────────────────────────────
    ry = body_top

    # TO DO — actions with checkboxes
    ry = draw_block_title(c, "To Do", ry, rx, right_col_w, accent=color)
    for task in data["action"]:
        ry = draw_task_row(c, ry, task, margin_x=rx,
                           content_w=right_col_w, accent=color)
    ry = draw_free_row(c, ry, margin_x=rx, content_w=right_col_w)

    # RESEARCH — bullets
    ry += 12
    ry = draw_block_title(c, "Research", ry, rx, right_col_w, accent=color)
    for item in data["research"]:
        ry = draw_bullet(c, item, ry, rx, right_col_w, color=color,
                         size=9.5, leading=13)
        ry += 3

    # CONSIDER — bullets
    ry += 10
    ry = draw_block_title(c, "Consider", ry, rx, right_col_w, accent=color)
    for item in data["consider"]:
        ry = draw_bullet(c, item, ry, rx, right_col_w, color=color,
                         size=9.5, leading=13)
        ry += 3

    # PROGRESS PHOTO — portrait box, centred in the column
    ry += 12
    ry = draw_block_title(c, "Progress Photo", ry, rx, right_col_w, accent=color)
    photo_w = right_col_w * 0.5
    photo_h = photo_w * 1.28
    px = rx + (right_col_w - photo_w) / 2
    c.setFillColor(PHOTO_BG)
    c.setStrokeColor(PHOTO_BORDER)
    c.setLineWidth(0.6)
    c.roundRect(px, rl(ry + photo_h), photo_w, photo_h, 4, fill=1, stroke=1)
    c.setFillColor(TEXT_SEC)
    c.setFont("Helvetica-Oblique", 8)
    c.drawCentredString(px + photo_w / 2, rl(ry + photo_h / 2) - 2,
                        "Add your photo")
    ry += photo_h + 12

    # HABITS
    ry = draw_block_title(c, "Habits", ry, rx, right_col_w, accent=color)
    ry = draw_habit_tracker(c, ry, margin_x=rx,
                            content_w=right_col_w, rows=3, accent=color)

    # NOTES — fills whatever remains
    ry += 12
    ry = draw_block_title(c, "Notes", ry, rx, right_col_w, accent=color)
    available = bottom_ty - ry
    note_count = max(2, int(available / NOTE_SPACING))
    draw_note_lines(c, note_count, ry, margin_x=rx, content_w=right_col_w)


# ─────────────────────────────────────────────────────────────────────
# APPOINTMENTS — section title page
# ─────────────────────────────────────────────────────────────────────
def _draw_section_cover(c, title, subtitle, active_idx, bookmarks,
                        bg=OAT, subtitle_italic=False, title_size=34):
    """Shared section title page: solid colour, sprig, serif display
    title, rule, subtitle, side tabs."""
    c.setFillColor(bg)
    c.rect(0, 0, PAGE_W, PAGE_H, fill=1, stroke=0)
    for b in bookmarks:
        c.bookmarkPage(b)
    ...
    draw_sprig(c, page_cx, PAGE_H / 2 + 120, color=white, scale=1.15,
               height=140)
    draw_display_title(c, title.upper(), page_cx, PAGE_H / 2 - 26,
                       size=title_size, color=white, font=SERIF)
    ...
    if subtitle:
        ...
        if subtitle_italic:
            c.setFont("Helvetica-Oblique", 12)
            c.drawCentredString(page_cx, PAGE_H / 2 - 76, subtitle)
        else:
            draw_tracked(c, subtitle.upper(), 0, PAGE_H / 2 - 76,
                         "Helvetica", 11, 1.3, color=TRIM_WHITE,
                         centre_x=page_cx)
...
def draw_appts_title_page(c):
    _draw_section_cover(c, "Appointments",
                        "Booking · Scans · Midwife", 4, ["dest_appts"])
...
def _appt_bullets(c, label, top_y, mx, cw, count=4):
    """A labelled section of empty bulleted write-on lines."""
    c.setFont("Helvetica-Bold", SEC_FONT)
    c.setFillColor(TEXT_SEC)
...
def _appt_next_steps_bullets(c, top_y, mx, cw, count=4):
    return _appt_bullets(c, "NEXT STEPS", top_y, mx, cw, count=count)
...
def draw_booking_appt(c):
    ...
    # Questions — bulleted
    cur_y = _appt_bullets(c, "QUESTIONS TO ASK", cur_y, mx, cw, count=5)
...
def _draw_scan_page(c, title, sublabel, dest_name, intro,
    ...
    cur_y += 14
    photo_side = min(cw, 248)
    px = mx + (cw - photo_side) / 2
    c.setFillColor(PHOTO_BG)
    c.setStrokeColor(PHOTO_BORDER)
    c.setLineWidth(0.6)
    c.roundRect(px, rl(cur_y + photo_side), photo_side, photo_side, 4,
                fill=1, stroke=1)
    c.setFillColor(TEXT_SEC)
    c.setFont("Helvetica-Oblique", 9)
    c.drawCentredString(px + photo_side / 2, rl(cur_y + photo_side / 2) - 3,
...
def draw_midwife_appt(c, week, dest, first_time_only):
    ...
    cur_y = _appt_bullets(c, "QUESTIONS TO ASK", cur_y, mx, cw, count=4)
...
def draw_checklists_title_page(c):
    _draw_section_cover(c, "Checklists",
                        "One step at a time", 5, ["dest_checklists"])


# ─────────────────────────────────────────────────────────────────────
# CHECKLIST HELPER
# ─────────────────────────────────────────────────────────────────────
def draw_checklist_item(c, text, top_y, mx, col_w, link=False, row_h=None):
    """Single checklist item with checkbox + wrapping text.
    row_h sets a minimum row height (used to space pages out)."""
    if row_h is None:
        row_h = TASK_ROW_H
...
def draw_hospital_bag(c):
    ...
    ly = ry = BODY_START + 10
    ROW = 25          # generous row height so the page fills the sheet
    GRP = 20          # gap between groups
    ...
    for item in [
        ...
        ly = draw_checklist_item(c, item, ly, lx, col_w, row_h=ROW)
    ...
    for item in [
        ...
        ry = draw_checklist_item(c, item, ry, rx, col_w, row_h=ROW)
...
def draw_baby_essentials(c):
    ...
    rx = MARGIN_X + col_w + gap
    ly = ry = BODY_START + 10
    ROW = 24          # generous row height so the page fills the sheet
    GRP = 20          # gap between groups
...
def draw_signs_of_labour(c):
    ...
    # These are things to recognise, not tasks to tick — so they are
    # presented as bullets rather than checkboxes.
    ...
    for item in [
        ...
        cur_y = draw_bullet(c, item, cur_y, mx, cw, color=CLAY, leading=16)
        cur_y += 8
    ...
    for item in [
        ...
        cur_y = draw_bullet(c, item, cur_y, mx, cw, color=TERRACOTTA, leading=16)
        cur_y += 8
...
def draw_birthplan_title_page(c):
    _draw_section_cover(c, "Birth Plan", "Vaginal or C-Section", 6,
                        ["dest_birthplan", "dest_birth_plan"])
...
def draw_birth_plan_vaginal_p1(c):
    ...
    right_y = save_y
    c.setFont("Helvetica", 8); c.setFillColor(TEXT_SEC)
    c.drawString(mx + col_w + 14, rl(right_y),
                 "ENVIRONMENT  e.g. soft lighting, music etc.")
    right_y += 12
    # Free space for the user to write their own environment preferences
    right_y = _bp_free_lines(c, 6, right_y, mx + col_w + 14, col_w)
    ...
    cur_y = _bp_free_lines(c, 2, cur_y, mx, cw)
    cur_y += 8
...
def draw_birth_plan_vaginal_p2(c):
    ...
    cur_y = _bp_free_lines(c, 3, cur_y, mx, cw)
...
def draw_birth_plan_postdelivery(c):
    ...
    cur_y = _bp_free_lines(c, 1, cur_y, mx, cw)
    cur_y += 6
...
def draw_journal_title_page(c):
    _draw_section_cover(c, "Journal",
                        "Words for the days you want to remember", 7,
                        ["dest_journal"], subtitle_italic=True)


def draw_notes_title_page(c):
    _draw_section_cover(c, "Notes", "", 8, ["dest_notes"])
...
def draw_closing_page(c):
    ...
    page_cx = (PAGE_W - TAB_W) / 2
    draw_sprig(c, page_cx, PAGE_H / 2 + 175, color=white, scale=1.0,
               height=120)

    draw_display_title(c, "AND HERE YOU ARE", page_cx, PAGE_H / 2 + 60,
                       size=24, color=white, font=SERIF)
...
# CONTENT DATA
# ─────────────────────────────────────────────────────────────────────
# Weekly to-dos from the Excel 'Weekly Tasks' sheet ("Claude generated
# to do list V1"): Actions (checkbox), Research and Consider (bullets).
# Weeks 1–3 are omitted (the planner starts at week 4).
WEEKLY = {
    4:  {"action": ["Book a GP appointment",
                    "Start folic acid (400mcg daily)"],
         "research": ["Vitamin D in pregnancy",
                      "Listeria and food safety"],
         "consider": ["When to tell your employer"]},
    5:  {"action": ["Register with your GP or midwife",
                    "Note your last menstrual period date"],
         "research": ["How the booking appointment works",
                      "NHS maternity care options"],
         "consider": ["Midwife-led or consultant-led care pathway?"]},
    6:  {"action": ["Book your midwife booking appointment (ideally by week 8 – 10)",
                    "Look into pregnancy multivitamins"],
         "research": ["What happens at the booking appointment",
                      "Pregnancy vitamins — what's needed"],
         "consider": ["NHS midwife, independent midwife, or private care"]},
    7:  {"action": ["Prepare questions for your booking appointment",
                    "Avoid alcohol, raw fish, soft cheeses, and raw meat"],
         "research": ["Hypnobirthing — what it is, and whether it interests you",
                      "Mental health support in pregnancy"],
         "consider": ["Who you want to attend scans and appointments with you"]},
    8:  {"action": ["Attend booking appointment (if booked)",
                    "Complete blood tests requested at booking"],
         "research": ["Down's, Edwards' and Patau's screening — how it works",
                      "NIPT (non-invasive prenatal testing) options"],
         "consider": ["Whether to opt into combined screening at 11 – 13 weeks"]},
    9:  {"action": ["Rest when you need to — fatigue is normal",
                    "Research maternity pay and leave entitlements"],
         "research": ["Statutory vs occupational maternity pay",
                      "Maternity Allowance if self-employed",
                      "Shared parental leave"],
         "consider": ["When to tell your employer (legal deadline is week 25)"]},
    10: {"action": ["Book your 12-week dating scan if not yet done",
                    "Think about who you want at your scan"],
         "research": ["What happens at the 12-week scan",
                      "How combined screening results are interpreted"],
         "consider": ["Who will be your primary support person?"]},
    11: {"action": ["Prepare questions for your sonographer",
                    "Note any symptoms to mention to your midwife"],
         "research": ["Signs of miscarriage and when to seek help",
                      "Ectopic pregnancy symptoms — worth knowing"],
         "consider": ["When to share your news more widely"]},
    12: {"action": ["Attend your 12-week dating scan",
                    "Review combined screening results with midwife"],
         "research": ["What 'low' and 'higher' risk screening results mean",
                      "NIPT if you want further reassurance"],
         "consider": ["Telling close family and friends after the scan"]},
    13: {"action": ["Book your anatomy scan",
                    "Consider sharing pregnancy news more widely"],
         "research": ["Second trimester — what to expect physically",
                      "Free NHS dental treatment in pregnancy"],
         "consider": ["When to tell your employer (must be by week 25)"]},
    14: {"action": ["Research maternity leave options in detail",
                    "Speak to HR about your company's maternity policy"],
         "research": ["Pram types — travel system vs pushchair vs pram",
                      "Car seat safety — infant carrier vs i-Size"],
         "consider": ["Start a list of big-ticket baby items to budget for"]},
    15: {"action": ["Start a baby names list if you'd like",
                    "Explore antenatal class options — popular ones book up early"],
         "research": ["NCT classes vs NHS antenatal classes",
                      "Online vs in-person antenatal options"],
         "consider": ["Nursery ideas — no pressure, just early inspiration"]},
    16: {"action": ["Attend midwife appointment",
                    "Think about your birth preferences — no rush"],
         "research": ["What to expect at the 16-week midwife appointment",
                      "Braxton Hicks vs real contractions"],
         "consider": ["You may start to feel early movements (quickening)"]},
    17: {"action": ["Book antenatal classes if not yet done",
                    "Research hospitals or birth centres nearby"],
         "research": ["Birth centre vs labour ward vs home birth",
                      "Water birth options"],
         "consider": ["Who will be your birth partner — have the conversation"]},
    18: {"action": ["Book your anatomy scan if not yet done",
                    "Start thinking about birth preferences formally"],
         "research": ["Prams and car seats — test in person if possible",
                      "Hypnobirthing courses — book early"],
         "consider": ["Start a hospital bag list (early is fine)"]},
    19: {"action": ["Prepare questions for your anatomy scan",
                    "Think about hospital bag basics"],
         "research": ["What the anatomy scan checks for",
                      "Placenta previa — what it means if detected"],
         "consider": ["Rest when you can — your body is growing fast now"]},
    20: {"action": ["Attend your anatomy scan",
                    "Note scan results and any follow-up needed"],
         "research": ["Fetal movements — when they become regular",
                      "Iron levels in pregnancy — dietary sources"],
         "consider": ["Celebrate — you're halfway there"]},
    21: {"action": ["Discuss scan results with your midwife",
                    "Start your hospital bag list formally"],
         "research": ["Baby monitors — audio vs video, smart options",
                      "Breastfeeding vs formula — explore both, no pressure"],
         "consider": ["Childcare — nurseries can have long waiting lists"]},
    22: {"action": ["Research childcare options and register interest on waiting lists",
                    "Look into your employer's enhanced maternity policy"],
         "research": ["Childcare costs and the 15 / 30 free hours scheme",
                      "Nursery, childminder, nanny — differences and costs"],
         "consider": ["Research prams — test a few in person if possible"]},
    23: {"action": ["Book antenatal classes if not yet started",
                    "Start a postpartum plan — meals, support, rest"],
         "research": ["The fourth trimester — what recovery looks like",
                      "Postpartum mental health — signs of PND"],
         "consider": ["Confirm your birth partner understands the role"]},
    24: {"action": ["Attend midwife appointment",
                    "Review your baby budget and priorities"],
         "research": ["Glucose tolerance test — if at risk of GD",
                      "Perineal massage — when and how to start"],
         "consider": ["What you truly need for the nursery vs nice-to-have"]},
    25: {"action": ["Notify employer of pregnancy (legal deadline)",
                    "Submit MATB1 form once received from midwife"],
         "research": ["The MATB1 form — what it is and when you get it",
                      "SMP eligibility criteria"],
         "consider": ["Draft your birth preferences — a first attempt"]},
    26: {"action": ["Submit maternity leave paperwork to HR",
                    "Confirm your last working day in writing"],
         "research": ["Shared parental leave — if relevant",
                      "Keeping-in-touch (KIT) days — what they are"],
         "consider": ["Start thinking about a work handover plan"]},
    27: {"action": ["Book a hospital or birth centre tour",
                    "Start gathering items for your hospital bag"],
         "research": ["Cord blood banking — what it is and costs",
                      "Group B Strep testing — private test from ~35 weeks"],
         "consider": ["Who will look after you postpartum — practical support"]},
    28: {"action": ["Attend midwife appointment (blood tests this week)",
                    "Review your birth preferences document"],
         "research": ["Anti-D injection (if Rhesus negative)",
                      "Iron supplements if levels are low"],
         "consider": ["Sleep may get harder — start prioritising rest now"]},
    29: {"action": ["Finalise nursery essentials purchases",
                    "Install or plan for baby's car seat"],
         "research": ["Car seat installation — ISOFIX vs belted",
                      "Safe sleep guidelines — The Lullaby Trust"],
         "consider": ["Check your hospital bag list — what's still needed?"]},
    30: {"action": ["Start packing your hospital bag",
                    "Confirm your birth partner's availability near your due date"],
         "research": ["Signs of labour — show, waters, contractions",
                      "When to call the hospital"],
         "consider": ["Postpartum meals — batch cook or arrange a meal train"]},
    31: {"action": ["Attend antenatal class if still in session",
                    "Talk through your birth preferences with your midwife"],
         "research": ["Pain relief in labour — gas and air, epidural, TENS",
                      "What a birth plan does and doesn't guarantee"],
         "consider": ["It's okay to slow down — physically and at work"]},
    32: {"action": ["Add key items to hospital bag",
                    "Pre-register at your hospital or birth centre"],
         "research": ["Newborn care basics — cord care, bathing, feeding cues",
                      "Colostrum harvesting — what it is, whether to try"],
         "consider": ["Discuss any anxieties about birth with your midwife"]},
    33: {"action": ["Finalise your birth preferences document",
                    "Prepare your home for baby's arrival"],
         "research": ["Caesarean section — what to expect, planned or emergency",
                      "Perineal massage — continue if started"],
         "consider": ["A list of people to notify after birth"]},
    34: {"action": ["Attend midwife appointment",
                    "Prepare list of people to notify after birth"],
         "research": ["Induction of labour — why it happens, what to expect",
                      "Fetal movements — what's normal at this stage"],
         "consider": ["Hospital bag should be nearly complete"]},
    35: {"action": ["Check hospital bag is complete",
                    "Know the signs of labour",
                    "Consider a private Group B Strep test"],
         "research": ["Group B Strep — risks and testing",
                      "What to do if your waters break at home"],
         "consider": ["Reduce commitments and rest where possible"]},
    36: {"action": ["Weekly midwife appointments begin from now",
                    "Install car seat and check fitting",
                    "Stock up on postpartum essentials for yourself"],
         "research": ["Postpartum recovery — what your body will need",
                      "Pelvic floor exercises — continue throughout"],
         "consider": ["Prepare a postpartum essentials basket for yourself"]},
    37: {"action": ["Final check on hospital bag",
                    "Inform your birth partner to stay on call"],
         "research": ["Early labour — what to do and when to go in",
                      "The BRAIN acronym for decisions in labour"],
         "consider": ["Full term — baby could arrive any time now"]},
    38: {"action": ["Attend weekly midwife appointment",
                    "Tie up any loose ends at home"],
         "research": ["Overdue pregnancy — monitoring and options",
                      "Membrane sweep — what it involves"],
         "consider": ["Rest as much as possible — a legitimate priority"]},
    39: {"action": ["Stay in contact with your midwife",
                    "Know when to call your midwife or hospital"],
         "research": ["Induction — what to expect if offered",
                      "Positions for labour — upright, all fours, etc."],
         "consider": ["Walk gently if it helps you feel settled"]},
    40: {"action": ["Stay in contact with your midwife",
                    "Rest, breathe, trust yourself"],
         "research": ["Post-dates monitoring — what happens after 40 weeks",
                      "Induction timeline if offered"],
...
T1_PRIORITIES = [
    ...
    "Self-refer to your chosen NHS Trust; check the NHS website for how to find Trusts near you.",
